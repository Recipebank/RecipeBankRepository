...
CREATE TABLE `category` (
  ...
  `CatagoryTitle` varchar(45) NOT NULL,
...
INSERT INTO `recipe` VALUES (1,'Chicken Salad','Fresh salad with grilled chicken',4,1,NULL),(2,'Macaroni & Cheese','Macaroni nooldes covered in cheddar cheese',2,1,NULL),(3,'Uncle \n\nSams Ribs','Uncle Sams bbq smoked ribs covered in special bbq sauce',5,1,NULL),(4,'Apple \n\nVinegrette','Salad Dressing',3,1,NULL),(5,'Pad \n\nThai','Chicken, Shrimp or Beef Pad Thai',2,1,NULL),(6,'Lemon \n\nButter Asparagus','Pan fried asparagus in butter with lemon and pepper',4,1,NULL),(7,'Meatballs','Simple meatball recipe',1,1,NULL),(8,'Aunt \n\nMarys Fish & Chips','Aunt Marys famous beer batter fish and sea salt fries',5,1,NULL),(9,'Scalloped Potatoes','Scalloped Potatoes',3,1,NULL),(10,'Garlic \n\nand Herb LoL','Leg of lamb roasted with organic herbs and garlic',3,1,NULL),(11,'fajita-style quesadillas','Hot and savory',4,0,NULL),(12,'oatmeal cookies','The recipes for oatmeal cookies.',3,0,NULL);
...
CREATE TABLE `recipecategory` (
  `RecipeCatagoryId` int(11) NOT NULL AUTO_INCREMENT,
  `RecipeId` int(11) NOT NULL,
  `CatagoryId` int(11) NOT NULL,
  PRIMARY KEY (`RecipeCatagoryId`),
  KEY `fk_recipecatagory_RecipeId_idx` (`RecipeId`),
  KEY `fk_recipecatagory_CatagoryId_idx` (`CatagoryId`),
  CONSTRAINT `fk_recipecatagory_CatagoryId` FOREIGN KEY (`CatagoryId`) REFERENCES `category` (`CatagoryId`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_recipecatagory_RecipeId` FOREIGN KEY (`RecipeId`) REFERENCES `recipe` (`RecipeId`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2013-10-31 17:28:37
